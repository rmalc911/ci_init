...
$(function () {
	...
	dtable = $("[data-ajax-url]").DataTable({
		...
		"dom": '<"row"<"col-4 pr-1"l><"col-8 pl-0"f>><"table-responsive border responsive-datatable-container"rt><"row"<"col-md-5"i><"col-md-7"p>>',
	...
	$("[data-visibility-name]").each(function () {
		...
		var vvalue = $(s).data('visibility-value');
					...
					swal.fire({
						...
						if (result.isConfirmed) {
							$.post({
								...
								success: function (res) {
									if (res.success) {
										$("#" + selectWidget).html(res.options.map(function (option) {
											return '<option value=' + option.option_value + '>' + option.option_name + '</option>';
										}).join(''));
										swal.fire(
											'Added!',
	...
	$("body").on('click', ".input-list-add", function () {
		...
		$(".input-list-serial", inputListItem).text(inputListLength + 1);
